    ...
    public void DrawNumber(Number number, PointF origo, bool lines)
    {
        ...
        if (lines)
        {
            foreach (Number parent in number.Parents)
            {
                ...
                Graph.DrawLine(linepen, x, y, targetx, targety);
                Graph.DrawLine(linepen, x, y, targetx + w, targety + w);
                Graph.DrawLine(linepen, x, y, targetx + w, targety - w);
                Graph.DrawLine(linepen, x, y, targetx - w, targety + w);
                Graph.DrawLine(linepen, x, y, targetx - w, targety - w);
            }
        ...
        //-- lines
        //-- fill
        Graph.FillEllipse(brushBackGround, rectEllipse);
        Graph.DrawEllipse(pen, rectEllipse);
        Graph.DrawString(str, font, brush, rectText, format);
    }
